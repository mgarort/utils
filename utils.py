def split_dataframe(df, split_fractions, shuffle=True, random_seed=None, filename_root=None):
    ...
    :df: dataframe to act on. Assumes that the label is the last column
    ...
    if shuffle:
        df = df.sample(frac=1,random_state=ranom_seed)
    ...
    val = df.loc[n_train:n_val]
    test = df.loc[n_train+n_val:n_test]
